...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS empty_rate(id INTEGER PRIMARY KEY, per_km REAL NOT NULL, active INTEGER NOT NULL DEFAULT 0, created_at TEXT);
    """)
    if conn.execute("SELECT COUNT(*) c FROM tariff").fetchone()["c"] == 0:
        ...
        conn.commit()
    if conn.execute("SELECT COUNT(*) c FROM empty_rate").fetchone()["c"] == 0:
        conn.execute("INSERT INTO empty_rate(per_km,active,created_at) VALUES (1.5,1,datetime('now'))")
        conn.commit()
    conn.close()
